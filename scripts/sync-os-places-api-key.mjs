#!/usr/bin/env node
/**
 * Push OS_PLACES_API_KEY to every hub Worker (or selected sites).
 *
 * Usage (from repo root):
 *   export OS_PLACES_API_KEY='…'
 *   node scripts/sync-os-places-api-key.mjs
 *   node scripts/sync-os-places-api-key.mjs --site test-lovely --site smith
 *   node scripts/sync-os-places-api-key.mjs --dry-run
 *
...
const SECRET_NAME = 'OS_PLACES_API_KEY';
const root = join(dirname(fileURLToPath(import.meta.url)), '..');
const workerPkgPath = join(root, 'worker/package.json');

function printHelp() {
  console.log(`Sync OS_PLACES_API_KEY to hub Workers (UK UPRN lookup for bin auto-import).

Usage:
  node scripts/sync-os-places-api-key.mjs [options]
...
Environment:
  OS_PLACES_API_KEY       Required — OS Data Hub Project API key (Places API enabled)
...
function main() {
  ...
  const secretValue = process.env.OS_PLACES_API_KEY?.trim();
  if (!secretValue && !options.dryRun) {
    console.error(
      'OS_PLACES_API_KEY is not set. Export the same value as the GitHub repo secret, or use --dry-run.'
    );
    process.exit(1);
  }
