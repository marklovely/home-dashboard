...
const SITTER_CONTROLS_KEY = 'sitter_controls_enabled';
 ...
 * @param {Record<string, string | undefined>} env
 */
export async function getSitterControlsManual(env) {
  const db = env.HOUSE_GUIDE_DB;
  if (!db) {
    return env.SITTER_CONTROLS_ENABLED?.trim() === '1';
  }

  const row = await db
    .prepare('SELECT value FROM house_settings WHERE key = ?')
    .bind(SITTER_CONTROLS_KEY)
    .first();
  return row?.value === '1';
}
 ...
 */
export async function setSitterControlsManual(env, enabled) {
  const db = requireHouseSettingsDb(env.HOUSE_GUIDE_DB);
  const now = Math.floor(Date.now() / 1000);
  await db
    .prepare(
    ...
    .bind(SITTER_CONTROLS_KEY, enabled ? '1' : '0', now)
    .run();
}
