...
describe('ownerAuthProvider', () => {
  ...
  it('returns unavailable without API base URL', async () => {
    ...
    await expect(ownerAuthProvider.authenticate('1234')).resolves.toBe('unavailable');
  });
  ...
  it('maps worker responses to UI results', async () => {
    ...
    for (const [response, expected] of cases) {
      ...
      await expect(ownerAuthProvider.authenticate('1234', fetchImpl)).resolves.toBe(expected);
    }
  });
});
