...
export async function browserAutocomplete(term, token, fetchImpl = fetch) {
  ...
  if (!response.ok) {
    const upstream = String(data?.Message ?? data?.message ?? '').trim();
    if (response.status === 401) {
      return {
        ok: false,
        message:
          upstream ||
          'Domain Token rejected. On getAddress.io, copy the Value from Domain Tokens (not the API Key) into GETADDRESS_DOMAIN_TOKEN on the smith Worker, then redeploy.'
      };
    }
    return {
      ok: false,
      message: upstream || `Address lookup failed (HTTP ${response.status}).`
    };
  }
