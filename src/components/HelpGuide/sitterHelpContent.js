...
export function buildSitterHelpSections() {
  const hosts = getHostDisplayName('your hosts');
  const stayPlace = getStayPlaceLabel();
  const petName = getPetCareAppTitle();
  const petVisible = isPetCareVisibleInHelp();
  const controlsVisible = isControlsVisibleInHelp();
  const petSummary = getPetSpeciesSummary();
  const isDemo = isDemoHubHelpContext();
      ...
      blocks: [
        {
          type: 'p',
          text: petVisible
            ? `This tablet is set up for your stay at ${stayPlace}. Everything you need is a tap away — caring for ${petName}, using appliances, and getting help if something goes wrong.`
            : `This tablet is set up for your stay at ${stayPlace}. Everything you need is a tap away — using appliances, finding your way around, and getting help if something goes wrong.`
        },
        {
          type: 'p',
          text: isDemo
            ? 'You are exploring a public demo hub. Sample data resets overnight. The bottom bar gives quick access to Home, House Guide, Emergency, and Settings.'
            : 'You do not need a password on this tablet. The bottom bar gives quick access to Home, House Guide, Emergency, and Settings.'
        },
        {
          type: 'ul',
          items: [
    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          type: 'p',
          text: isDemo
            ? 'Some topics include Wi‑Fi, address, lockbox code, or contact details in highlighted boxes. On this demo hub those details are sample data only.'
            : `Some topics include Wi‑Fi, address, lockbox code, or contact details in highlighted boxes. ${hosts} turn these on when you arrive — if a box says details are not available yet, ask them or check Emergency. On Wi‑Fi → Connecting (or QR Code), scan the QR code with your phone camera to join the network once details are shared.`
        },
        {
          type: 'h4',
      ...
      blocks: [
        ...
        },
        {
          ...
          text: isDemo
            ...
            : 'The tablet is meant to answer “how do I…?” without you needing to remember where everything is. If a page or photo does not match what you see in the house, call us — we would rather know.'
        }
      ]
    }
  );
