...
Provisioning runs **one platform hub at a time** (`max-parallel: 1` + `platform-terraform-state-platform`) because those hubs share `platform.tfstate`. Customer hubs (`*.lovely-hub.com`) each have `home-dashboard/customers/{siteId}.tfstate`, so two household signups can Terraform-apply in parallel. R2 still has no DynamoDB lock, so two applies on the **same** state file must not overlap — CI uses `platform-terraform-state-customers-{siteId}` for that.
...
Registry PRs (`platform-site-manage`) lock **per hub**, not globally. GitHub concurrency is not a queue: one shared group keeps a single pending job and **cancels** the rest, which is how four overlapping signups only opened two PRs. Different households run in parallel; overlay onto `origin/main` plus a reconcile when another `platform:` PR merges (and a one-minute cron) replay leftover **conflicting or behind** PRs. Squash auto-merge will not land a branch that is only behind `main`, so overlay must run in that case too — not only on merge conflicts. Terraform apply retries Cloudflare **429**s; a **403** account limit (for example D1 databases per account on the Workers Free plan) fails immediately instead of waiting through four retries. A Durable Object or Cloudflare Queue would be a real FIFO if we ever move the registry off git; while `sites.yaml` lives in this repo, GitHub Actions is the right writer and a bad waiter.
...
| `home-dashboard/customers.tfstate` | Household hubs on `lovely-hub.com` (combined file — backup after the next split) |
| `home-dashboard/hub.tfstate` | Left in place as a backup — **do not apply** against it |

### Split customer Terraform state (per site)

After this change is on `main`, run **Split customer Terraform state** once (`confirm` = `split-sites`). That copies each `module.hub_site["id"]` out of `customers.tfstate` into:

| R2 key | Contents |
|--------|----------|
| `home-dashboard/customers/{siteId}.tfstate` | One household hub |
| `home-dashboard/customers.tfstate` | Left in place as a backup — **do not apply** against it |

Provision/deprovision **fail closed** if a platform backend is still empty while yaml expects sites, or a per-site customers backend still looks like the combined estate. Do not run overlapping customer provisions until the per-site migrate workflow is green.

Local `backend.hcl` must use `platform.tfstate` or `customers/{siteId}.tfstate` and you must pass `-var=terraform_stack=platform` (or `customers`) on every plan/apply. Customer applies also need `-var=provision_site_id={siteId}`.
...
**Sandbox (or another hub) loses `HUB_API` after a merge to `main`:** Hub Pages projects keep a GitHub source for previews but **production git deploys are off**. Provision wrangler-deploys the hub; app-code changes on `main` run [deploy-hub-pages.yml](../.github/workflows/deploy-hub-pages.yml). Registry-only merges must not rebuild every household (that queue starved platform Pages and made signup `registered` lag). Existing projects are patched by `scripts/disable-hub-pages-git-production.mjs` from **Ensure hub HUB_API bindings**. If a binding is still missing: `node scripts/ensure-hub-api-bindings.mjs sandbox` then `bash scripts/deploy-cloudflare-pages-site.sh sandbox`.
...
4. `set-worker-secrets-from-terraform.mjs` — HUB_PROXY_SECRET, Access AUD, vanilla dummy secrets, optional `GOOGLE_PLACES_API_KEY` (address lookup), optional `OS_PLACES_API_KEY` (UK UPRN / bin auto-import)
...
| [`platform-sync-os-places-key.yml`](../.github/workflows/platform-sync-os-places-key.yml) | Manual — push `OS_PLACES_API_KEY` to all hub Workers (or one site) |
